...
## v5.12（2026-08-21 · `3086a38`）

### 调整
- **宠物放大一倍**：`PET_DRAW` 40→80，肥波/肥强游戏内绘制尺寸翻倍（帧为 64×64 归一化精灵，放大后 80×80）。
- **肥波尿渍区域放大三倍**：`PUDDLE_R` 42→126，主斑/湿润高光/卫星小滴/伤害减速判定半径同步放大三倍（直径 84→252px）。
- **配套**：宠物屏幕钳制边距 `M` 28→40（对齐放大后宠物半宽），避免放大后宠物重新超出屏幕（v5.2 已修过的问题）。
...
### 修复
- **肥波尿液仍看不到、只见到冲撞（v5.9 修复未生效的根因）**：v5.9 的迁移用了**一次性标志 `_starterPetV57`**，用户一旦在契约界面选过肥强（或该标志被某次加载置位），迁移便**永久不再覆盖**，`selectedPet` 卡死在 `amer` → 永远只见肥强冲撞、看不到肥波尿液。修复：去掉一次性标志，改为「用户未在契约界面主动选过宠物（`_petExplicitChoice`）前，强制出战初始肥波(orange)；用户主动选过任意宠物后尊重其选择（可随时切回肥波）」。Node 仿真 8 场景全过（含老存档卡肥强被强制改回肥波）。另在 `PetSystem.update` 加 `window.__petDebug` 运行时探针（active/shots/puddles/cd），便于真机排查。

---

## v5.9（2026-08-21 · `3534f70`）

### 修复
- **肥波尿液攻击看不到、只见到冲撞（疑似与肥强攻击模式搞混）**：先抓线上 bundle 验证——`night-survivors.pages.dev` 当前 JS 含 `初始自带`/`#ffe066`/`#e8c21e`/`drawHazards` 等 v5.6+ 全部特征，旧弱渲染色 `#f2d94a`/`#e6c230` 计数 0，**部署未卡旧版本、攻击模式代码未搞混**（orange=urine、amer=butt）。根因是**出战宠物实为肥强(amer)而非肥波(orange)**：v5.7 把肥波设为初始自带时，`getSelectedPet` 仅在「从未选过宠物」时默认肥波；用户在 v5.5 强制选宠时若选了肥强，`selectedPet='amer'` 为真不会被覆盖 → 一直带肥强、只见冲撞。修复：在 `migrateSouls` 加一次性迁移，老存档若曾选肥强则改回肥波出战（置 `_starterPetV57` 标志，迁移后用户仍可在契约界面手动切回肥强）。Node 仿真 7 场景全过。
...
### 修复
- **肥波尿液攻击仍看不见（飞行抛物线 + 地面尿渍均不可见）**：根因是渲染太弱且被暗角渐变压暗——飞溅弹体只是 `6×5` 小黄点（看不出抛物线），地面水洼是 `alpha 0.30` 的规整椭圆、又叠在 vignette 之下被进一步压暗。修复：
  - **飞行尿液**：改为带**抛物线拖尾**（记录最近 16 帧轨迹、渐隐黄线）+ **发光弹体**（径向渐变光晕 + 实心黄核），清晰呈现抛物轨迹；
  - **地面尿渍**：改为**不规则黄色斑渍**（落地时一次性生成随机顶点主斑，压扁呈地面泼洒感）+ 暗边描边 + 湿润高光 + 3–5 颗卫星小滴，不再是规整椭圆；
  - **绘制层级**：尿液危害层（`drawHazards`）从 `pets.draw` 拆出，**移到暗角渐变之后绘制**（`game.js` 渲染末尾），确保无论屏幕位置都清晰可见；猫本体仍按原层级（`drawPet`）。

---

## v5.5（2026-08-21 · `a71661c`）

### 修复
- **肥波尿液抛物线攻击不生效**：攻击触发被死卡 `宠物须贴到敌人 70px 内`，但宠物被 `FOLLOW_LEASH=240` 钳制在血裔 240px 内、敌人只在血裔 `ENGAGE_RANGE=180` 内才被感知 → 宠物几乎永远到不了敌人 70px → 尿液（远程抛物线喷射）几乎从不触发。改为：敌人在 `ENGAGE_RANGE` 内即发动（尿液抛物线喷射 / 美短头撞冲撞均远程结算），并放大尿液飞溅弹道（6×5 + 描边）提升可见性。
- **宠物商店删除「无宠物」卡片**：`renderPet` 移除「不带宠物」选项卡，玩家只能从已解锁宠物中选择出战。

---

## v5.4（2026-08-21 · `3651162`）

### 修复
- **宠物完全不可见（PC 端看不到宠物跟随血裔）**：根因是渲染坐标未转屏幕坐标。游戏 `render()` 已对 ctx 应用相机平移（`-cam.ox / -cam.oy`），所有实体都用**世界坐标 - cam.ox/oy** 转屏幕坐标绘制（`player.render` 等）；而 `pets.draw(ctx)` 调用时**未传 `cam`**，宠物 `draw` 直接用世界坐标 `this.x/this.y`，导致在已平移的 ctx 上再叠加世界坐标 → 宠物被画到屏幕外。修复：`PetSystem.draw`/`Pet.draw` 接收 `cam` 并改用 `x - cam.ox / y - cam.oy`，水洼与尿液飞溅同步修正；`game.js` 调用处补传 `cam`。
...
### 修复
- **宠物选中失效（PC 端点「选择」无反应）**：`loadSouls()` 的 base 对象漏写 `selectedPet` 持久化字段（对比 `selectedBloodline` 已包含），导致每次读取 localStorage 时丢弃已保存的宠物选择 → `getSelectedPet()` 恒返 null → 卡片永远不显示「使用中」。修复：base 补上 `selectedPet: o?.selectedPet ?? null`。
- **美短猫图标黑花纹融入暗底**：宠物卡缩略图背景原为 `rgba(0,0,0,.5)`，深色美短猫毛在其上低对比几乎不可见。改为浅中性渐变背景 `linear-gradient(180deg,#a59ec0,#6a6488)`。同时给游戏内宠物本体加柔和浅色背光（径向渐变），避免深色猫在黑夜场内不可见。
- **宠物帧肚皮下方白色背景残留**：`flood_key_bg` 用 `min>246` 严格阈值做边缘泛洪，肚子下方的背景白因轻微渐变/阴影落在 233–246 区间（非纯白 250+），泛洪过不去「稍暗接缝」导致残留；`_flood_remove_connected_white` 兜底也用同阈值，同样漏删。修复：改为**两阶段泛洪——种子严格（min>246 纯白）+ 蔓延放宽容差（距纯白≤22 即亮度≥233 且 min>215）**：从纯白背景越过暗接缝吃掉肚皮下残留白，而被深色身体包围、不连通边界的猫白毛不受波及。诊断确认 13 帧残留背景白全部归零，猫身白毛像素健康保留。
- **宠物帧腿缝纯白岛残留（猫腿之间白色背景）**：`flood_key_bg` 的两阶段泛洪（含 bridge 借道穿透）只清得掉「被暗边细缝阻断的连通背景」，但猫腿之间的背景白是**被猫腿实心封死的纯白孤岛**（在原始图上本就不连通四边，任何连通泛洪都进不去）。诊断确认：13 帧共残留 `min>250` 纯白孤岛（橘猫 pickup_0 达 6692px、美短 follow_1 达 1564px）。修复：在 `remove_bg` 后新增 `_fill_white_holes`——删除所有「不连通四边 + 纯白 `min>250` 且距纯白≤5」的孤立块（腿缝残白），保留 240–249 的猫身白毛（直方图已确认背景纯白峰 250–255、猫白毛峰 240–249）。诊断确认 13 帧残留纯白岛合计 = **0**，猫白毛（240–249）保留 5205px。
- **宠物帧白毛被误删（全局近白清理吃掉猫身白花纹）**：`gen_pet_frames.py` 的 `remove_bg` 第二步用**全局** `r,g,b>180` 删除所有近白像素，不区分背景与猫身白毛 → 银渐层美短的白底、橘猫的白色胸腹花纹全部被清零（13 帧白像素占比 0%）。根因：旧版 `flood_key_bg` 用「距四角容差 40」做泛洪，猫白毛（~240）距纯白（255）差值 15<40，一并被吃；再叠加全局清理彻底抹零。修复：改用**绝对亮度阈值 `min(r,g,b)>246`** 判定背景（纯白峰 250-255 vs 猫白毛峰 240-249），边缘泛洪只吃纯白背景 + 连通性清理只删与背景连通的近纯白残留 → 白毛完整保留（美短 13–24%、橘猫 4–15%，肉眼确认银渐层虎纹+橘猫白胸均可见）。

### 新增
- **微信域名验证文件**：`public/f3cf20e70ac09eb3e2c94fba4342e616.txt`（微信分享链接风控验证）。

---

## v5.0（2026-08-20 · `3c0112c`）
...
### 修复
- **永夜使徒无初始武器导致开局死锁**（`d4981cb`）：真机试玩反馈选永夜使徒开局后无法攻击怪物、无法获得经验升级，整局卡死。
  - **根因**：`data.js` 中 `apostle` 血裔 `weapon: null`，而 `game.js:281` 的 `if (bl.weapon)` 因 null 为假跳过发武器；其 `apply` 只加属性、技能树「使徒权能」节点也只加伤害/减伤，全程没有任何攻击手段。游戏的升级经济完全依赖击杀掉落经验（`game.js:471/518/523 → gainExp`），无武器 → 无法击杀 → 无经验 → 无法升级 → 无法开宝箱选武器，构成**开局硬死锁**。
  - **修法（方案 A · 槽外固有权能）**：复用既有 `innate` 机制（圣徒圣水同款），将 `apostle` 改为 `weapon: 'aura', innate: true`，开局获得「永夜光环（亡灵光环复用，槽外固有·不占武器槽）」。玩家既能攻击/击杀/升级/开宝箱，又保留「无武器（槽）起手」的差异化身份（0 个武器槽被占用，仍可正常捡/升级获得常规武器）。
  - **设计权衡**：原「无武器起手·高难高回报」意图成立，但缺少起手攻击手段导致高难直接变不可玩。本次仅补起手攻击能力，原属性偏向（伤害+30% / 移速+25% / 冷却-25% / 生命-20%）保持不变；其强度是否需随固有光环重新校准，留待真机试玩数据判定（平衡数值多标 `[PLACEHOLDER]`）。

---

## v4.3.5（2026-08-10 · `88fd903`）

### 修复
- **永夜化身（终局 Boss）太亮，不符合暗哥特风格**（`88fd903`）：真机试玩反馈 `boss_avatar` 亮度 143/255（高亮像素 33%），是所有 Boss 中最亮的，与整体暗哥特风格脱节。
  - **根因**：P7 终局 Boss 独立立绘时 ImageGen prompt 偏亮（高调发光/霓虹感），且陈旧源图 `.ai_monster_raw/avatar/raw.png` 已缺失。
  - **修法**：ImageGen 重绘——迭代 3 版选 **cand3（暮光紫罗兰+强轮廓光+发光眼/虚空冠）**。经 `gen_monster_pixels.py` 标准怪物管线（众数色键控 + NEAREST 2x + 描边 + auto_brighten 兜底）输出。
  - **效果**：
    | 指标 | 旧（太亮版） | 新（v4.3.5） |
    |------|-------------|-------------|
    | 游戏内亮度 | 143/255 | **82/255** ✅ |
    | 高亮(>180) 像素 | 33% | **9%** ✅ |
    | 暗部(<40) 像素 | 27% | **35%** ✅ |
    | 彩色占比 | 67% | **99%** ✅ |
    | 图鉴黑底可见性 | 过亮刺眼 | **清晰可辨** ✅ |
  - **设计约束**：用户要求"不要太亮、符合风格、也不要太黑、避免图鉴异常" → 暮光紫罗兰中暗色调 + 轮廓光保证图鉴可见 + 仅眼/冠发光。

---

## v4.3.4（2026-08-07 · `2141d7a`）

### 修复
- **apostle（永夜使徒）游戏内太黑**（`2141d7a`）：真机试玩反馈 apostle 在游戏中几乎纯黑，无法辨识角色形象。
  - **根因**：ImageGen 源图 prompt 偏"近黑虚空剪影"，生成灰阶稿（亮度≈30/255，死黑像素占 37%），后处理无法凭空造色。
  - **修法**：用 ImageGen 高保真重绘——迭代 3 版（v1 深紫靛蓝 → v2 明亮符文粒子 → v3 紫水晶星云半透明高调发光），选 v3（综合分 90 vs v2 的 41）覆盖源图。经 `gen_portrait_pixels.py` + `gen_bloodline_pixels.py` 双管线输出。
  - **效果**：
    | 指标 | v4.3.1/v4.3.3（旧） | v4.3.4（新） |
    |------|------|------|
    | 游戏内 player 亮度 | ~30–42/255 | **82/255** |
    | 死黑像素占比 | 70%+ | **19%** |
    | 彩色像素占比 | 3–10% | **95%** |
    | 视觉特征 | 纯黑剪影 | 紫水晶星云 + 品红能量环 + 青白眼 |

---

## v4.3.3（2026-08-07 · `e60dc49`）

### 修复
- **血裔游戏内精灵与选择卡片形象不一致**（`e60dc49`）：真机试玩反馈卡片与游戏内仍不一致、不清晰、apostle 白噪点、流浪者缩放偏小。
  - **根因**：游戏内 `player_*.png` 走 `gen_monster_pixels.py`（方形源图 + 众数色键控 + NEAREST 2x 像素化 + 1px 描边），而卡片走 `gen_portrait_pixels.py`（竖版源图 + 边缘泛洪 + LANCZOS 平滑）——两套不同 ImageGen 生成 + 不同管线，导致四个问题。
  - **修法**：新建 `gen_bloodline_pixels.py`，复用卡片的 `.ai_portrait_raw/<id>/raw.png` 源图 + 同一套边缘泛洪 + LANCZOS 管线，输出 64×64 归一化精灵。从架构上保证"同源 = 天然一致"。
  - **效果量化**：
    | 问题 | 修复前 | 修复后 |
    |------|--------|--------|
    | 卡片/游戏内主色差 Δ | 72–151 | **1.0–2.4** |
    | apostle 内部白噪点 | **313** | **0** |
    | wanderer 占画布 | 42%（28×62）| 32%（21×64，全身归一化，与卡片一致）|
    | 管线 | 怪物管线（NEAREST+描边）| 卡片管线（LANCZOS平滑，无描边）|
